...
import { findPosition, random, roll } from '../../util/math'
...
export const Branch: React.FC<BranchProps> = (props) => {
  ...
  const spawnBranches = (start: Vector3) => {
    ...
    if (roll(constants.rootSurvival)) spawnAsyncBranch(<Branch key={uuid()} start={start} xAngle={0} zAngle={0} diaStart={diaEnd} diaEnd={nextBranchDia(diaEnd)} len={nextBranchLen(len)} depth={nextDepth} />)

    if (roll(constants.sideSurvival)) spawnAsyncBranch(<Branch key={uuid()} start={start} xAngle={-3} zAngle={0} diaStart={diaEnd} diaEnd={nextBranchDia(diaEnd)} len={nextBranchLen(len)} depth={nextDepth} />)
    if (roll(constants.sideSurvival)) spawnAsyncBranch(<Branch key={uuid()} start={start} xAngle={3} zAngle={0} diaStart={diaEnd} diaEnd={nextBranchDia(diaEnd)} len={nextBranchLen(len)} depth={nextDepth} />)
    if (roll(constants.sideSurvival)) spawnAsyncBranch(<Branch key={uuid()} start={start} xAngle={0} zAngle={3} diaStart={diaEnd} diaEnd={nextBranchDia(diaEnd)} len={nextBranchLen(len)} depth={nextDepth} />)
    if (roll(constants.sideSurvival)) spawnAsyncBranch(<Branch key={uuid()} start={start} xAngle={0} zAngle={-3} diaStart={diaEnd} diaEnd={nextBranchDia(diaEnd)} len={nextBranchLen(len)} depth={nextDepth} />)
  }
  ...
  return (
    <mesh
      ...
      position={start}
    >
      <cylinderGeometry ref={ref.geometry} args={[diaEnd, diaStart, length, Math.max(3, Math.floor(diaStart * constants.SideMultiplier)), 1, false]} />
